...
public class ItemStackSerializer implements ObjectSerializer<ItemStack> {
    ...
    private ItemMeta getMeta(ItemMeta meta, DeserializationData data) {
        ...
        if(data.containsKey("lore")) {
            List<String> lore = data.get("lore", ArrayList.class);
            meta.setLore(ColorUtils.getColored(lore));
        }
    ...
    private ItemStack getItem(DeserializationData data) {
        String material = (Objects.equals(data.get("material", String.class), "DynamicMaterial")) ? "SKULL" : data.get("material", String.class);
        int durability;
        int amount;
